...
class Candito6Week extends StatefulWidget {
  final Program program;
  final String unit;

  const Candito6Week({super.key, required this.program, required this.unit});

  @override
  _Candito6WeekState createState() => _Candito6WeekState();
}

class _Candito6WeekState extends State<Candito6Week> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (week == 1) {
      if (session == 1) {
        // Week 1, Session 1: Hypertrophy (Squat, Bench)
        exercises.add({
          'name': 'Squat',
          'sets': 3,
          'reps': 10,
          'weight': _programLogic.calculateWorkingWeight(oneRMs['Squat'] ?? 0.0, 0.65, week, 10, session),
        });
        exercises.add({
          'name': 'Bench Press',
          'sets': 3,
          'reps': 10,
          'weight': _programLogic.calculateWorkingWeight(oneRMs['Bench'] ?? 0.0, 0.65, week, 10, session),
        });
      } else if (session == 2) {
        // Week 1, Session 2: Hypertrophy (Deadlift, Bench)
        exercises.add({
          'name': 'Deadlift',
          'sets': 3,
          'reps': 8,
          'weight': _programLogic.calculateWorkingWeight(oneRMs['Deadlift'] ?? 0.0, 0.65, week, 8, session),
        });
        exercises.add({
          'name': 'Bench Press',
          'sets': 3,
        ...
        // Week 1, Session 3: Hypertrophy (Squat, Bench)
        exercises.add({
          'name': 'Squat',
          'sets': 3,
          'reps': 10,
      ...
      if (session == 1) {
        // Week 2, Session 1: Hypertrophy (Squat, Bench)
        exercises.add({
          'name': 'Squat',
          'sets': 4,
          'reps': 8,
          'weight': _programLogic.calculateWorkingWeight(oneRMs['Squat'] ?? 0.0, 0.70, week, 8, session),
        });
        exercises.add({
          'name': 'Bench Press',
          'sets': 4,
          'reps': 8,
          'weight': _programLogic.calculateWorkingWeight(oneRMs['Bench'] ?? 0.0, 0.70, week, 8, session),
        });
      } else if (session == 2) {
        // Week 2, Session 2: Hypertrophy (Deadlift, Bench)
        exercises.add({
          'name': 'Deadlift',
          'sets': 4,
          'reps': 6,
          'weight': _programLogic.calculateWorkingWeight(oneRMs['Deadlift'] ?? 0.0, 0.70, week, 6, session),
        });
        exercises.add({
          'name': 'Bench Press',
          'sets': 4,
        ...
        // Week 2, Session 3: Hypertrophy (Squat, Bench)
        exercises.add({
          'name': 'Squat',
          'sets': 4,
          'reps': 8,
      ...
      if (session == 1) {
        // Week 3, Session 1: Power (Squat, Bench)
        exercises.add({
          'name': 'Squat',
          'sets': 3,
          'reps': 3,
          'weight': _programLogic.calculateWorkingWeight(oneRMs['Squat'] ?? 0.0, 0.80, week, 3, session),
        });
        exercises.add({
          'name': 'Bench Press',
          'sets': 3,
          'reps': 3,
          'weight': _programLogic.calculateWorkingWeight(oneRMs['Bench'] ?? 0.0, 0.80, week, 3, session),
        });
      } else if (session == 2) {
        // Week 3, Session 2: Power (Deadlift, Bench)
        exercises.add({
          'name': 'Deadlift',
          'sets': 3,
          'reps': 3,
          'weight': _programLogic.calculateWorkingWeight(oneRMs['Deadlift'] ?? 0.0, 0.80, week, 3, session),
        });
        exercises.add({
          'name': 'Bench Press',
          'sets': 3,
        ...
        // Week 3, Session 3: Power (Squat, Bench)
        exercises.add({
          'name': 'Squat',
          'sets': 3,
          'reps': 3,
    ...
    workoutDetails = {
      'week': week,
      'session': session,
      'workoutName': 'Candito 6 Week Day $session',
      'exercises': exercises,
      'unit': widget.unit,
    };
