...
class CustomProgram extends StatefulWidget {
  ...
  const CustomProgram({super.key, required this.program, required this.unit});

  @override
  _CustomProgramState createState() => _CustomProgramState();
}

class _CustomProgramState extends State<CustomProgram> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    final workouts = _programLogic.getWorkoutsForCurrentDay();
    final List<Map<String, dynamic>> exercises = [];

    for (var workout in workouts) {
      final weight = _programLogic.calculateWorkingWeight(
        widget.program.oneRMs[workout['name']] ?? 0.0,
        0.9,
        widget.program.currentWeek,
        workout['reps'] as int,
    ...
    workoutDetails = {
      ...
      'session': widget.program.currentSession,
      'workoutName': 'Custom Program Day ${widget.program.currentSession}',
      'exercises': exercises,
      'unit': widget.unit,
    };
