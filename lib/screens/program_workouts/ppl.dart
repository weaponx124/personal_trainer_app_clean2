...
class PPL extends StatefulWidget {
  ...
  const PPL({super.key, required this.program, required this.unit});

  @override
  _PPLState createState() => _PPLState();
}

class _PPLState extends State<PPL> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (session == 1) {
      ...
      exercises.add({
        ...
        'sets': 3,
        'reps': 5,
        'weight': (oneRMs['Bench'] ?? 0.0) * 0.9,
      });
      exercises.add({
        'name': 'Overhead Press',
        'sets': 3,
        'reps': 8,
        'weight': (oneRMs['Overhead'] ?? 0.0) * 0.7,
      });
    } else if (session == 2) {
      // Pull Day
      exercises.add({
        ...
        'sets': 3,
        'reps': 5,
        'weight': (oneRMs['Deadlift'] ?? 0.0) * 0.9,
      });
      exercises.add({
        'name': 'Barbell Row',
        'sets': 3,
        'reps': 8,
        'weight': (oneRMs['Row'] ?? 0.0) * 0.7,
      });
    } else if (session == 3) {
      // Legs Day
      exercises.add({
        ...
        'sets': 3,
        'reps': 5,
        'weight': (oneRMs['Squat'] ?? 0.0) * 0.9,
      });
      exercises.add({
        'name': 'Leg Press',
        'sets': 3,
        'reps': 10,
        'weight': (oneRMs['Squat'] ?? 0.0) * 0.6,
      });
    }
