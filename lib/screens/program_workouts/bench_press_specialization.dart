...
class BenchPressSpecialization extends StatefulWidget {
  final Program program;
  final String unit;

  const BenchPressSpecialization({super.key, required this.program, required this.unit});

  @override
  _BenchPressSpecializationState createState() => _BenchPressSpecializationState();
}

class _BenchPressSpecializationState extends State<BenchPressSpecialization> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    final benchWorkingWeight = _programLogic.calculateWorkingWeight(
      oneRMs['Bench'] ?? 0.0,
      0.9,
      week,
      3,
    ...
    final deadliftWorkingWeight = _programLogic.calculateWorkingWeight(
      oneRMs['Deadlift'] ?? 0.0,
      0.9,
      week,
      3,
    ...
    final overheadWorkingWeight = _programLogic.calculateWorkingWeight(
      oneRMs['Overhead'] ?? 0.0,
      0.9,
      week,
      3,
    ...
    final benchAccessoryWeight = _programLogic.calculateWorkingWeight(
      oneRMs['Bench'] ?? 0.0,
      0.7,
      week,
      8,
    ...
    final squatAccessoryWeight = _programLogic.calculateWorkingWeight(
      oneRMs['Squat'] ?? 0.0,
      0.7,
      week,
      8,
    ...
    final deadliftAccessoryWeight = _programLogic.calculateWorkingWeight(
      oneRMs['Deadlift'] ?? 0.0,
      0.7,
      week,
      8,
    ...
    if (session == 1) {
      ...
      exercises.add({
        ...
        'sets': 3,
        'reps': 3,
        'weight': benchWorkingWeight,
      });
      exercises.add({
        'name': 'Incline Bench Press',
        'sets': 3,
        'reps': 8,
        'weight': benchAccessoryWeight,
      });
      exercises.add({
        'name': 'Overhead Press',
        'sets': 3,
        'reps': 3,
        'weight': overheadWorkingWeight,
      });
    } else if (session == 2) {
      // Squat Day
      exercises.add({
      ...
      exercises.add({
        ...
        'sets': 3,
        'reps': 3,
        'weight': deadliftWorkingWeight,
      });
      exercises.add({
        'name': 'Romanian Deadlift',
        'sets': 3,
        'reps': 8,
        'weight': deadliftAccessoryWeight,
      });
    }
    ...
    workoutDetails = {
      'week': week,
      'session': session,
      'workoutName': 'Bench Press Specialization Day $session',
      'exercises': exercises,
      'unit': widget.unit,
    };
